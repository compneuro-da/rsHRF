...
parser = ArgumentParser(description='retrieves the onsets of pseudo-events triggering a hemodynamic response from resting state fMRI BOLD voxel-wise signal')
...
if args.bids_dir is not None and args.atlas is not None:
    ...
    all_inputs = layout.get(modality='func', task='rest', type='preproc', extensions=['nii', 'nii.gz'])
    if not all_inputs != []:
...
if args.bids_dir is not None and args.brainmask:
    layout = BIDSLayout(args.bids_dir)
    all_inputs = layout.get(modality='func', task='rest', type='preproc', extensions=['nii', 'nii.gz'])
    all_masks = layout.get(modality='func', task='rest', type='brainmask', extensions=['nii', 'nii.gz'])
