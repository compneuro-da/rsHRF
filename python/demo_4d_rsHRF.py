...
para['lag'] = np.arange(np.fix(min_onset_search/para['dt']),np.fix(max_onset_search/para['dt'])+1,dtype='int')
...
for isub in range(len(sub)):
    ...
    if(v1.header.get_data_shape()[:-1]!=v.header.get_data_shape()):
        ...
        plt.plot(TR*np.arange(1,np.amax(hrfa[:,0].shape)+1),hrfa[:,0],linewidth=1)
        plt.xlabel('time (s)')
        plt.show()
