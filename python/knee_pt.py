...
def knee_pt(y,x=None):
    ...
    if type(y) is not np.ndarray:
        ...
        if np.size(y)==0:
            ...
            bfwd = -1*((np.multiply(sigma_x,sigma_xy) - np.multiply(sigma_xx,sigma_y)) / det)
            ...
            n = np.arange(1, np.amax(y.shape) + 1).conj().T
            det = np.multiply(n,sigma_xx) - np.multiply(sigma_x,sigma_x)
            mbck = ((np.multiply(n,sigma_xy) - np.multiply(sigma_x,sigma_y)) / det)[::-1]
            bbck = (-1*((np.multiply(sigma_x,sigma_xy) - np.multiply(sigma_xx,sigma_y)) / det))[::-1]

            error_curve = np.full(y.shape,np.nan)
            for breakpt in range(1,np.amax((y-1).shape)):
                delsfwd = (np.multiply(mfwd[breakpt],x[0:breakpt+1]) + bfwd[breakpt]) - y[0:breakpt+1]
                delsbck = (np.multiply(mbck[breakpt],x[breakpt:]) + bbck[breakpt]) - y[breakpt:]
